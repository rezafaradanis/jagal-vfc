...
async function kirimKeDiscord(webhook, mm) {
  ...
  const pencetakGol = potong(pemain.filter((p) => p.gol > 0).map((p) => `⚽ ${p.nama} (${p.gol})`).join('\n'));
  const pemberiAssist = potong(pemain.filter((p) => p.assist > 0).map((p) => `🅰️ ${p.nama} (${p.assist})`).join('\n'));
  ...
  const rincian = potong(pemain.map((p) => {
    ...
    return `${p.mom ? '⭐ ' : ''}${p.nama} — ${bagian.join(' · ')}`;
  }).join('\n'));
  ...
  if (motm) fields.push({ name: 'Man of the Match', value: `⭐ ${motm.nama} (${motm.rating.toFixed(1)})`, inline: true });
